...
static void csi2_frame_timer_cb(void *opaque)
{
    ...
    if (!QTAILQ_EMPTY(&s->dma_queue)) {
        ...
        /* Use pre-initialized buffer, only update frame counter */
        /* Buffer is already filled with Y=16, U/V=128 (valid black frame) */
        uint32_t *counter = (uint32_t *)s->dma_buf;
        *counter = s->frame_count; /* First 4 bytes as frame counter */
        
        /* Write frame to guest memory */
        if(pci_dma_write(PCI_DEVICE(s), req->addr, s->dma_buf, s->frame_size) != 0){
...
static void amd_csi2_rx_pcie_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    /* Initialize once with valid YUV black frame */
    for (int i = 1; i < FRAME_SIZE; i += 2) {
        s->dma_buf[i] = 128; /* U/V channels */
    }
